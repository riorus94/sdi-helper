...
def _write_dataset_pose_yaml(num_keypoints: int) -> pathlib.Path:
    """Write a temporary pose dataset yaml with absolute path."""
    flip_idx = [
        0, 2, 1, 4, 3, 7, 8, 5, 6, 10, 9, 11, 13, 12, 15, 14, 16, 17, 18
    ] if num_keypoints == 19 else list(range(num_keypoints))
...
def train() -> None:
    ...
    dataset_yaml = _write_dataset_pose_yaml(num_keypoints)
